...
class PU21Encoder:
    ...
    uniform (PU) space. This class adapts image quality metrics to work
    on HDR content.

    Refer to the PU21 paper:
    R. Mantiuk and M. Azimi, Picture Coding Symposium 2021
    """
    ...
    def __init__(self, encoding_type='banding_glare'):
        ...
        if encoding_type == 'banding':
            ...
            raise ValueError(f"Unknown type: {encoding_type}")

    def encode(self, Y):
        """
        Convert from linear (optical) luminance Y to encoded (electronic) values V.
        Y must be in absolute units (nits). Returns V in a perceptually uniform space.
        """
        epsilon = 1e-5
    ...
    def decode(self, V):
        """
        Convert from encoded (electronic) values V back to linear (optical) luminance Y.
        """
        p = self.par
        V_p = np.maximum(V / p[6] + p[5], 0) ** (1 / p[4])
        numerator = np.maximum(V_p - p[0], 0)
        denominator = p[1] - p[2] * V_p
        Y = (numerator / denominator) ** (1 / p[3])
        return Y
